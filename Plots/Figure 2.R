...
tiff("Plots\\Figure 2.tiff", width=11, height=5, units="in", res=500)
...
#######################################################################################################################################
par(mfrow=c(3,3), mar=c(0,3,0,0), oma=c(5,3,3,2))
load("Data\\Deterministic Run Nproj25\\Deterministic Projections.Rdata")
minY <- c(15, 1, 5)
# Deterministic Plot
	for (y in 1:3) {
		maxY <- max(maxYY[y])*1.15
		...
		plot(1, type="n", xlab="", ylab=ylabel, 
			xlim=c((ProjStart[y]),(ProjStart[y]+nyr-1)), ylim=c(minY[y],maxY),
...
for (yy in 1:3) {
	...
	for (y in 1:3) {
		...
		# plot(1, type="n", xlab="", ylab=ylabel, 
		# 	xlim=c((ProjStart[y]-1), (ProjStart[y]+nyr-1+1)), ylim=c(0,50), xaxt="n",
		# 	yaxt=ytemp, xaxs="i", yaxs="i", main=topmain,
		# 	cex=1.5, cex.main=1.5, cex.lab=1.5, cex.axis=1.5)

		# if(y==1) {
		# 	tix <- seq(from=0, to=maxY, by=50)
		# 	axis(side=2, at=tix[-1], labels=tix[-1], outer=F)
		# 	mtext(text="OFL (mt)", side=2, line=3, outer=T)
		# }

		# if (y==2) {
		# 	tix <- seq(from=0, to=maxY, by=5)
		# 	axis(side=2, at=tix[-1], labels=tix[-1], outer=F)
		# }

		# if (y==3) {
		# 	tix <- seq(from=0, to=maxY, by=25)
		# 	axis(side=2, at=tix[-1], labels=tix[-1], outer=F)
		# }
		...
		plot(1, type="n", xlab="", ylab=ylabel, 
			xlim=c((ProjStart[y]), (ProjStart[y]+nyr-1)), ylim=c(minY[y],(max(Ymax)*1.15)), xaxt="n",
			yaxt=ytemp, xaxs="i", yaxs="i", main=topmain,
			cex=1.5, cex.main=1.5, cex.lab=1.5, cex.axis=1.5)
...
for (yy in 1:3) {
	...
	for (y in 1:3) {
		...
		# plot(1, type="n", xlab="", ylab=ylabel, 
		# 	xlim=c((ProjStart[y]-1), (ProjStart[y]+nyr-1+1)), ylim=c(0,maxY),
		# 	xaxt="n", yaxt=ytemp, xaxs="i", yaxs="i", main=topmain,
		# 	cex=1.5, cex.main=1.5, cex.lab=1.5, cex.axis=1.5)

		# tix <- seq(from=(ProjStart[y]-1), to=(ProjStart[y]+nyr-1+1), by=2)
		# ntix <- length(tix)
		# axis(side=1, at=tix[c(-1,-ntix)], labels=tix[c(-1,-ntix)], outer=T)

		# if (y==1) {
		# 	tix <- seq(from=0, to=maxY, by=100)
		# 	axis(side=2, at=tix[-1], labels=tix[-1], outer=F)
		# }
		...
		if(y==1) {
			tix <- seq(from=0, to=(max(Ymax)*1.15), by=20)
			axis(side=2, at=tix[-1], labels=tix[-1], outer=F, cex.axis=1.5)
			mtext(text="OFL (mt)", side=2, line=1, outer=T, cex=1.5)
		}
